		...
		if ( isset( $_REQUEST['param'] ) ) { // WPCS: input var ok.
			...
			switch ( sanitize_text_field( wp_unslash( $_REQUEST['param'] ) ) ) { // WPCS: CSRF ok, WPCS: input var ok.
				...
				case 'update_database_entries_mail_bank':
					...
					break;
				case 'remove_banner_mail_bank':
					if ( wp_verify_nonce( isset( $_REQUEST['_wp_nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wp_nonce'] ) ) : '', 'remove_mail_bank_banner' ) ) { // WPCS: input var ok.
							update_option( 'mail_bank_banner_coupon', 'mail_bank_banner_coupon' );
					}
					break;
