...
class Mail_Bank_Zend_Validate_EmailAddress extends Mail_Bank_Zend_Validate_Abstract {
	...
	const INVALID_LOCAL_PART = 'emailAddressInvalidlocal_part';
	...
	protected $message_templates = array(
		...
		self::DOT_ATOM           => "'%local_part%' can not be matched against dot-atom format",
		self::QUOTED_STRING      => "'%local_part%' can not be matched against quoted-string format",
		self::INVALID_LOCAL_PART => "'%local_part%' is not a valid local part for email address '%value%'",
	...
	public function setMessage( $message_string, $message_key = null ) {
		if ( null === $message_key ) {
			$this->_options['hostname']->setMessage( $message_string );
			parent::setMessage( $message_string );
			return $this;
		}

		if ( ! isset( $this->message_templates[ $message_key ] ) ) {
			$this->_options['hostname']->setMessage( $message_string, $message_key );
		}
	 ...
	 * @param Mail_Bank_Zend_Validate_Hostname $hostname_validator OPTIONAL.
	 * @param int                              $allow  OPTIONAL.
	 * @return $this
	 */
	public function setHostnameValidator( Mail_Bank_Zend_Validate_Hostname $hostname_validator = null, $allow = Mail_Bank_Zend_Validate_Hostname::ALLOW_DNS ) {
		if ( ! $hostname_validator ) {
			$hostname_validator = new Mail_Bank_Zend_Validate_Hostname( $allow );
		}

		$this->_options['hostname'] = $hostname_validator;
	 ...
	 * @return boolean
	 */
	private function is_reserved( $host ) {
		...
		if ( (int) $octet[0] >= 224 ) {
			return true;
		} elseif ( array_key_exists( $octet[0], $this->invalid_ip ) ) {
			foreach ( (array) $this->invalid_ip[ $octet[0] ] as $subnet_data ) {
				// we skip the first loop as we already know that octet matches.
				for ( $i = 1; $i < 4; $i++ ) {
					if ( strpos( $subnet_data, $octet[ $i ] ) !== $i * 4 ) {
						break;
					}
				}

				$host        = explode( '/', $subnet_data );
				$binary_host = '';
				$tmp         = explode( '.', $host[0] );
				for ( $i = 0; $i < 4; $i++ ) {
					$binary_host .= str_pad( decbin( $tmp[ $i ] ), 8, '0', STR_PAD_LEFT );
				}

				$segment_data = array(
					'network'   => (int) $this->to_ip( str_pad( substr( $binary_host, 0, $host[1] ), 32, 0 ) ),
					'broadcast' => (int) $this->to_ip( str_pad( substr( $binary_host, 0, $host[1] ), 32, 1 ) ),
				);

				for ( $j = $i; $j < 4; $j++ ) {
					if ( (int) $octet[ $j ] < $segment_data['network'][ $j ] ||
					(int) $octet[ $j ] > $segment_data['broadcast'][ $j ] ) {
						return false;
					}
				}
			}
	 ...
	 *
	 * @param string $binary MANDATORY.
	 * @return mixed
	 */
	private function to_ip( $binary ) {
		$ip  = array();
	 ...
	 * @return boolean
	 */
	private function validate_local_part() {
	 ...
	 * @return boolean
	 */
	private function validate_mx_records() {
		$mx_hosts = array();
		...
		$result = getmxrr( $hostname, $mx_hosts );
		if ( ! $result ) {
	 ...
	 * @return boolean
	 */
	private function validate_hostname_part() {
		$hostname = $this->_options['hostname']->setTranslator( $this->getTranslator() )->is_valid( $this->_hostname );
		if ( ! $hostname ) {
			$this->_error( self::INVALID_HOSTNAME );

			// Get messages and errors from hostnameValidator.
			foreach ( $this->_options['hostname']->get_messages() as $code => $message ) {
			...
			$hostname = $this->validate_mx_records();
		}
	...
	public function is_valid( $value ) {
		...
		if ( ( strlen( $this->local_part ) > 64 ) || ( strlen( $this->_hostname ) > 255 ) ) {
			$length = false;
		...
		if ( $this->_options['domain'] ) {
			$hostname = $this->validate_hostname_part();
		}

		$local = $this->validate_local_part();
